...
public class GetGraph {
     ...
     */
    private DOTExporter<EntityNode, EventEdge> createExporter() {
        DOTExporter<EntityNode, EventEdge> exp = new DOTExporter<>(new EntityIdProvider());
    ...
    public void exportGraph() throws Exception {
        DOTExporter<EntityNode, EventEdge> exporter = createExporter();
        GenerateGraph();
    ...
    /*
     * input should be file path, process name + pid or source IP:source Port->
     * destination ip:Port
     * type : the legal input for this parameter only includes "Network", "File",
     * "Process"
     */
    ...
    private void bfs(EntityNode start, Queue<EntityNode> queue, String input) throws IOException {
        DirectedPseudograph<EntityNode, EventEdge> subgraph = new DirectedPseudograph<EntityNode, EventEdge>(
                EventEdge.class);
        ...
        while (!queue.isEmpty()) {
            ...
            for (EventEdge o : outgoing) {
                ...
                if (edgeSet.add(o)) {
                    // EventEdge newEdge = new EventEdge(subcur, newTarget,
                    // o.getID(),o.getStartTime(),o.getEndTime());
            ...
            for (EventEdge i : incoming) {
                ...
                if (edgeSet.add(i)) {
                    // EventEdge newEdge = new EventEdge(newSource, subcur,
                    // i.getID(),i.getStartTime(),i.getEndTime());
        ...
        while (!queue.isEmpty()) {
            ...
            while (!queue.isEmpty()) {
                ...
                for (EventEdge e : incoming) {
                    if (e.getEndTime().compareTo(edge.getEndTime()) < 0 && removeDuplicate.add(e)) { // here need to
                                                                                                     // check!!!!
                        nextStep.offer(e);
                    }
                }
    ...
    public void backTrackWithHopCount(String input, String type, int hopcount) {
        ...
        while (!queue.isEmpty() && hopcount >= 1) {
            ...
            hopcount--;
            try {
                exporter.exportGraph(subgraph,
                        new FileWriter(String.format("backTrack_%s_%d_ouput.dot", "test", level)));
            } catch (IOException e) {
                System.out.println("IO Exception");
            }
            level++;
