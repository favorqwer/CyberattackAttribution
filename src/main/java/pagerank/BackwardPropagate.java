...
public class  BackwardPropagate {
    ...
    BackwardPropagate (DirectedPseudograph<EntityNode, EventEdge> input){
        graph = input;
    ...
    public void calculateWeights(){
        ...
        for(EntityNode n: vertexSet){
            ...
            if(amount < 1e-8){
                ...
                // Calculate total weight in order to normalize
                for(EventEdge outEdge: outEdges){
                    wTotal += outEdge.timeWeight*0.1 + outEdge.structureWeight*0.4+ outEdge.amountWeight*0.5;
                }

                for (EventEdge outEdge: outEdges){
                    outEdge.weight = (0.1*outEdge.timeWeight + 0.4*outEdge.structureWeight + 0.5*outEdge.amountWeight)/wTotal;
                    weights.put(outEdge.id, outEdge.weight);
                }
            }
        }
    ...
    public void calculateWeights_ML(boolean normalizeByOutEdges,int mode){
        System.out.println("calculateWeights_ML invoked: "+normalizeByOutEdges);
        Set<EntityNode> vertexSet = graph.vertexSet();
        //initializeWeights();
...
//                System.out.println("After normalization " + inEdge.weight);
    ...
    public void calculateWeights_ML_dec(boolean normalizeByOutEdges,int mode){
...
//                System.out.println("After normalization " + inEdge.weight);
    ...
    public void calculateWeights_Fanout(boolean normalizeByOutEdges){
